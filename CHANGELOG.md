...
- **Remote WAL writer prototype** — adds an experimental `:remote-wal` writer backend with a conditional remote WAL head (memory and S3/Tigris CAS), concrete datom WAL entries, replay on connect, local/remote materialization helpers, and explicit guards for unsupported branch operations. (0.8.TODO, [#TODO])
...
- **Remote WAL nil local-cache commits ignored** — startup no longer treats a local snapshot with a missing WAL commit id as matching a nil remote head/materialized head, preventing speculative cache state from being used as a recovery base. (0.8.TODO, [#TODO])
- **Remote WAL local-cache validation** — startup now validates local cache snapshot summaries against the remote WAL before trusting them, falling back to WAL replay when a cache entry claims the right commit id but contains different state. (0.8.TODO, [#TODO])
...
- **Remote WAL materialization head validation** — remote materialization now revalidates the WAL head after flushing indexes and before compacting pending entries, avoiding writes over a concurrently corrupted or replaced head. (0.8.TODO, [#TODO])
- **Remote WAL entry-id validation** — remote-WAL heads now reject pending entries whose commit id does not match the deterministic hash of the WAL entry content. (0.8.TODO, [#TODO])
...
- **Remote WAL replay max-eid preservation** — concrete WAL replay now preserves WAL summary `:max-eid`, including transactions that allocate ref-only tempids with no datom in entity position. (0.8.TODO, [#TODO])
...
- **Remote WAL failed-transaction catch-up** — remote-WAL writers now publish remote commits they replay before speculative validation, so a transaction rejected after another writer's commit does not leave the connection pinned to an older WAL head. (0.8.TODO, [#TODO])
- **Remote WAL merge guard** — `merge-db`/`merge-db!` now fail clearly in `:remote-wal` mode before dispatching to the writer, matching the unsupported branch-operation contract. (0.8.TODO, [#TODO])
...
- **Remote WAL transaction function guard** — `:remote-wal` now fails clearly when automatic CAS retry is enabled and tx data invokes transaction functions, avoiding silent repeated execution of potentially non-idempotent functions. (0.8.TODO, [#TODO])
- **Remote WAL writer option validation** — invalid `:wal-auto-materialize?` and `:wal-max-retries` values now fail during config load instead of surfacing later as writer-loop errors. (0.8.TODO, [#TODO])
...
- **Remote WAL create robustness** — `create-database` now connects to pre-existing remote stores/buckets, reports WAL-head conflicts via CAS, and cleans up the just-created local cache when remote initialization fails. (0.8.TODO, [#TODO])
